...
public class VSensSlider : MonoBehaviour
{
    [SerializeField]
    VSensInputField m_SensInput;
    ...
    void OnEnable()
    {
        ...
        float sens = GameUserSettingsManager.Instance?.VerticalSensitivityMultiplier ?? 1f;
    ...
    public void ChangeSens(float value)
    {
        ...
        m_SensInput.InputField.text = sens.ToString("F1");
        GameUserSettingsManager.Instance.VerticalSensitivityMultiplier = sens;
        m_SensInput.InputField.onSubmit.AddListener(m_SensInput.ChangeSens);
    }
}
